...
class QRadarClient:
    ...
    async def get_sgm_alert_count(self, hours: int = 24) -> int:
        """Son N saatte SGM ön ekli ve belirli log kaynaklarindan gelen alarm sayisini dondurur."""
        aql = (
            "SELECT COUNT(*) AS count FROM events "
            "WHERE QIDNAME(qid) ILIKE 'SGM -%' "
            "AND (logSourceId='63' OR logSourceId='2073' OR logSourceId='2433' OR logSourceId='17317') "
            f"LAST {hours} HOURS"
        )
        try:
            ...
            logger.error(f"get_sgm_alert_count AQL error: {e}")
            return 0
